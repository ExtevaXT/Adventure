 ...
 * @param <E> This
 */
public class ChargeAttack<E extends OgreEntity> extends DelayedBehaviour<E> {
    ...
    public void calculateTarget(E entity) {
        ...
        this.chargeVelocity = targetPos.subtract(entityPos).normalize().multiply(moveSpeed, 0, moveSpeed).subtract(0, entity.getAttributeValue(ForgeMod.ENTITY_GRAVITY.get()), 0);
    ...
    protected void tick(E entity) {
        if (this.delayFinishedAt <= entity.level.getGameTime()) {
            ...
            for (LivingEntity target : EntityRetrievalUtil.<LivingEntity>getEntities(entity.level, entity.getBoundingBox().expandTowards(this.chargeVelocity), target -> target != entity && target.isAlive() && target instanceof LivingEntity && (!(target instanceof Player pl) || !pl.isCreative()))) {
                ...
                target.setDeltaMovement(target.getDeltaMovement().add(entity.position().vectorTo(target.position()).scale((1) * 1.5f)));
